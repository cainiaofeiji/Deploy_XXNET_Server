...
try:
    ...
    try:
        ...
        println(u'说明：如果直接上传失败，可以先运行 Shadowsocks 然后再上传，如果您的 Shadowsocks 已经成功运行，请按回车键继续。')
        println(u'注意：上传时请保持 Shadowsocks 在默认端口 (1080) 监听（亲，随便在网上找个免费 SS 就可以上传了！）')
        raw_input()
...
注意：appid 请勿包含 ios/android/mobile 等字样，否则可能被某些网站识别成移动设备。
        '''.strip())
    main()
    println(os.linesep + u'亲，上传成功啦，请不要忘记把你的 APPID 写到配置文件，谢谢。按回车键退出程序。')
    raw_input()
